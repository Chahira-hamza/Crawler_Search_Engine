...
public class QueryProcessor {
    ...
    public static double GetPageRank_DB(int DID) throws SQLException {
        ...
        PreparedStatement GetMaxStmt = null;
        
        String GetMax = "SELECT Max (linkRank * pageRank) from Docs_URL";
        String PageR = " Select 0.2 *  linkRank * pageRank from Docs_URL where ID= ?";
        
        con.setAutoCommit(false);
        GetPageRStmt = con.prepareStatement(PageR);
        GetMaxStmt = con.prepareStatement(GetMax);
        
        GetPageRStmt.setInt(1, DID);
        
        ResultSet rs = GetPageRStmt.executeQuery();
        ResultSet ms = GetMaxStmt.executeQuery();
...
public static void GetDoc_Word(String IN, int numDocs) throws SQLException {
    ...
    // change the main to the following process function to use it in other class as an object
