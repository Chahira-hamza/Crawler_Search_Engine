...
public class crawler_v2 {
    ...
    private static HashSet<CustomURL> visited;
...
private void checkAndAdd(Elements links,CustomURL url) throws Exception
{
    ...
    for (Element link : links) 
    {
        if (!(link.attr("abs:href").equals("")) && !(link.attr("abs:href") == null) )
        {
            ...
            if (isUrlValid(urlc) && !(urlc.equals(url)) )
            {
                // synchronized
                addasExtracted(urlc);
                Document doc = null;
...
private void addasExtracted(CustomURL url)
{
    if ((currentIteration%2) != 0)
    {
        addtoCrawled(url);
...
private synchronized void addtoExtracted(CustomURL url)
{
    extracted.add(url);
}

private synchronized void addtoCrawled(CustomURL url)
{
    crawled.add(url);
}
